...
export default function MyApplications() {
  ...
  const allApps = Array.isArray(appsData) ? appsData : (appsData?.data ?? [])
          ...
          <button key={f} onClick={() => setFilter(f)}
            className={clsx('px-4 py-2 rounded-xl text-xs font-bold border whitespace-nowrap transition-all capitalize',
              filter === f
                ...
                : 'bg-white text-slate-500 border-slate-200 hover:border-blue-DEFAULT'
            )}>
            {f === 'All' ? `All (${allApps.length})` : `${f} (${counts[f]})`}
          </button>
        ))}
      </div>
        ...
        <div className="card overflow-hidden divide-y divide-slate-100">
          {filtered.map((app) => {
            ...
            const date       = app.applied_at || app.created_at
              ? new Date(app.applied_at || app.created_at).toLocaleDateString('en-KE', { day:'numeric', month:'long', year:'numeric' })
              : ''
              ...
              <div key={app.id} className="p-5 hover:bg-slate-50/80 transition-colors">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-center text-xl flex-shrink-0">
                    {type === 'internship' ? '💼' : type === 'part-time' ? '⏰' : type === 'contract' ? '📝' : '🎯'}
                  </div>
