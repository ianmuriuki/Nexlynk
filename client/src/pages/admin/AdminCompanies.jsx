...
import { Search, X, CheckCircle, XCircle, Building2 } from 'lucide-react'
...
export default function AdminCompanies() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:  () => adminAPI.companies(params).then(r => r.data.data ?? r.data),
  ...
  // data is now the array directly after unwrap
    ...
    <div className="p-6 lg:p-8 max-w-screen-xl">
      ...
      <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} className="mb-7">
        ...
        <p className="text-slate-500 text-sm mt-1">
          ...
          {counts.pending > 0 && (
            <span className="ml-2 text-warning font-semibold">· {counts.pending} pending approval</span>
          )}
        </p>
      </motion.div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? <Skeleton /> : filtered.length === 0 ? (
                ...
                // Server returns registered_at (aliased from u.created_at)
                const dateRaw    = co.registered_at || co.created_at
                const date       = dateRaw ? new Date(dateRaw).toLocaleDateString('en-KE', { day:'numeric', month:'short', year:'numeric' }) : '—'
                    ...
                    <tr key={co.id} className={clsx('hover:bg-slate-50/80 transition-colors', isExpanded && 'bg-blue-50/30')}>
                      <td className="px-5 py-4 cursor-pointer" onClick={() => setExpanded(isExpanded ? null : co.id)}>
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-navy flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                            {initials}
                          </div>
                          <div>
                        ...
                        <td colSpan={6} className="px-5 pb-5 pt-2">
                          <div className="bg-white rounded-xl border border-blue-100 p-5 grid sm:grid-cols-3 gap-5">
                            <div>
                              ...
                              <p className="text-sm text-slate-700">{co.contact_name || co.email}</p>
                              <p className="text-sm text-slate-500 mt-1">{co.contact_phone || '—'}</p>
                              <p className="text-sm text-slate-500">{co.contact_email || co.email}</p>
                            </div>
                            <div>
                              ...
                              {co.website && (
                                <a href={co.website} target="_blank" rel="noreferrer"
                                  className="text-sm text-blue-DEFAULT hover:underline">{co.website}</a>
                              )}
                            </div>
                            <div>
